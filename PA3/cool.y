    ...
    */
	...
	feature : OBJECTID '(' formal_list ')' ':' TYPEID '{' expr '}'
  	  ...
  	  | error ';'
    	{
        	...
        	SET_NODELOC(@1);
        	// 创建一个错误的方法作为占位符
        	$$ = method(idtable.add_string("error_method"),
                   	nil_Formals(),
                   	idtable.add_string("Object"),
                   	no_expr());
    	}
	...
	formal : OBJECTID ':' TYPEID
    	...
    	| error IN
    	{
        	@$ = @1;
        	SET_NODELOC(@1);
        	$$ = int_const(idtable.add_string("0"));
    	}
    	;
	...
	| OBJECTID '(' expr_list ')'
    	{
        	...
        	SET_NODELOC(@1);
        	$$ = dispatch(
            		object(idtable.add_string("self")), // 隐式self调用
            		$1, 
            		$3
        	);
   	}
	...
	| LET OBJECTID ':' TYPEID let_rest
	{
    		@$ = @1;
    		SET_NODELOC(@1);
    		// 这里 $5 已经是处理好的嵌套 let 表达式
    		$$ = let($2, $4, no_expr(), $5);
	}
	| LET OBJECTID ':' TYPEID ASSIGN expr let_rest
	{	
    		@$ = @1;
    		SET_NODELOC(@1);
    		// 这里 $6 已经是处理好的嵌套 let 表达式
    		$$ = let($2, $4, $6, $7);
	}
